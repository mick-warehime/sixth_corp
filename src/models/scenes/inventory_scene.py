...
class InventoryScene(Scene):
    ...
    def _update_layout(self):
        ...
        for slot in fillable_slots[:3]:
            data = _slot_data(slot, chassis)
            num_rows += _DEFAULT_ROWS_PER_SLOT + 1
            slot_elems_0.extend([(_slot_layout(data), _DEFAULT_ROWS_PER_SLOT),
                                 (None, 1)])
        assert slot_elems_0
        ...
        for slot in fillable_slots[3:]:
            data = _slot_data(slot, chassis)
            num_rows += _DEFAULT_ROWS_PER_SLOT + 1
            slot_elems_1.extend([(_slot_layout(data), _DEFAULT_ROWS_PER_SLOT),
                                 (None, 1)])
        storage_data = _slot_data(SlotTypes.STORAGE, chassis)
        storage_rows = storage_data.num_filled + 1
        slot_elems_1.append((_slot_layout(storage_data, storage_rows),
                             storage_rows))
...
def _slot_layout(slot_data: SlotHeader, num_rows: int = None) -> Layout:
    """Vertical layout storing a single slot's data."""

    # First row is just basic slot information
    elements = [(slot_data, 1)]
    elements.extend((SlotData(mod, False), 1) for mod in slot_data.mods)
...
def _slot_data(slot: SlotTypes, chassis: Chassis) -> SlotHeader:
